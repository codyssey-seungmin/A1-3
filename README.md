...
기분 · 좋아하는 장르 · 오늘 읽을 수 있는 시간을 입력하면, Google Gemini AI가 조건에 맞는 책 3권과 추천 이유를 알려주는 웹 서비스입니다.
...
- AI 책 추천: 기분·장르·시간 입력 → Gemini API 호출 → 책 3권 추천 결과 출력
...
| AI | Google Gemini API (`google-genai` 파이썬 패키지) |
| 배포 | GitHub + Vercel |
...
├── api/
│   └── recommend.py     # Gemini API 연동 서버리스 함수
├── docs/
...
## 배포 방법 (Vercel)
1. [Google AI Studio](https://aistudio.google.com/apikey)에서 무료로 API 키를 발급받는다.
...
## 환경 변수 설정
- 로컬 개발 시: `.env.example`을 복사해 `.env` 파일을 만들고, `GEMINI_API_KEY` 값을 채워 넣는다. (`.env`는 `.gitignore`에 등록되어 있어 커밋되지 않는다)
